class RemoveUnusedAttributes < ActiveRecord::Migration[7.0]
  def change
    ...
    remove_column :migration_steps, :association_selections, :text if column_exists?(:migration_steps, :association_selections)
    remove_column :migration_steps, :polymorphic_associations, :text if column_exists?(:migration_steps, :polymorphic_associations)
